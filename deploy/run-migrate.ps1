#
# One-shot: run `php artisan migrate --force` on the production server.
...
$tpl   = Get-Content (Join-Path $PSScriptRoot '_run_migrate.php.tpl') -Raw
$runnerLocal = Join-Path $PSScriptRoot "_run_migrate_$token.php"
($tpl -replace '__TOKEN__', $token) | Set-Content -Path $runnerLocal -Encoding ASCII

$runnerRemoteName = "_run_migrate_$token.php"
$remote = "/public_html/admin/public/$runnerRemoteName"

$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$logDir    = Join-Path $PSScriptRoot 'logs'
$script    = Join-Path $logDir "migrate-$timestamp.txt"
$log       = Join-Path $logDir "migrate-$timestamp.log"
...
$cleanupScript = Join-Path $logDir "migrate-cleanup-$timestamp.txt"
$cleanupLines = @(
